#!/usr/bin/env node
/**
 * Negative-bias synthetic CSV generator
 *
 * Generates rows with:
 *  - sale_price in [minPrice, maxPrice]
 *  - assessed_value = sqrt(sale_price) (deterministic, no noise)
 *
 * Usage:
 *   node generate_negative_bias_data.js [--rows N] [--min 10000] [--max 3000000]
 *                                       [--seed 12345] [--outfile generate_negative_bias.csv]
 *                                       [--dist uniform|log]
 *
 * Defaults:
 *   rows   = 1000
...
function generate({ rows, min, max, dist, rng }) {
  ...
  for (let i = 0; i < rows; i++) {
    ...
    // Assessed value = sqrt(price) (deterministic)
    const assessed = Math.round(Math.sqrt(price));
